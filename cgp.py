...
class Individual(object):
    ...
    def __init__(self, net_info, init):
        # 从self.gene的大小来看，确实genotype的编码里不含input node
        ...
        if not self.is_active[n]:
            ...
            for i in range(in_num):
                if self.gene[n][i+1] >= self.net_info.input_num:
                    ...
                    # input node默认不在编码内，从这里是可以看出来的，因为他扣掉了input_num的个数
    ...
    def __upsample_calc(self, n):
        if n < 0:     # input node
            return 0
        if n >= self.net_info.node_num:     # output node
            return self.__upsample_calc(self.gene[n][1] - self.net_info.input_num)
        if self.gene[n][0] < 9:             # intermediate node: upsample module
            return 1 + self.__upsample_calc(self.gene[n][1] - self.net_info.input_num)
        if self.gene[n][0] >= 9:            # intermediate node: sum / concat module
            return max(self.__upsample_calc(self.gene[n][1] - self.net_info.input_num), self.__upsample_calc(self.gene[n][2] - self.net_info.input_num))


    def check_upsample(self):
        return self.__upsample_calc(self.net_info.node_num+self.net_info.out_num-1)
    ...
    def mutation(self, mutation_rate=0.01):
        # 先确认发生变异的位置是不是在active的位置，是的话说明对结果会有影响
        # 然后变异结束以后更新一次新的genotype中哪些位置是active的
        active_check = False
        ...
        for n in range(self.net_info.node_num + self.net_info.out_num):
            ...
            col = np.min((int(n / self.net_info.rows), self.net_info.cols))
            # connection gene的编码里的整数并不是直接指下标，而是加上input node的数字
            max_connect_id = col * self.net_info.rows + self.net_info.input_num
            min_connect_id = (col - self.net_info.level_back) * self.net_info.rows + self.net_info.input_num \
                if col - self.net_info.level_back >= 0 else 0
    ...
    def active_net_list(self):
        ...
        #np.cumsum即为求前缀和，self.is_active是一维数组，表示一个gene是否被激活(对应着一个intermediate node 和 connection nodes)
        #active_cnt前input_num个是从0到input_num-1的数字，从表示function node和output node开始，记录的之前到现在是激活节点的累计个数
        ...
        for n, is_a in enumerate(self.is_active):
            if is_a:
                ...
                #从开头到这个intermediate/function node的输入节点的累计激活节点个数
...
class CGP(object):
    def __init__(self, net_info, eval_func, lam=4, imgSize=32, init=False):
        ...
        self.num_gen = 0
        self.num_eval = 0
        self.max_upsample_num = int(math.log2(imgSize) - 2)
        # 512x512 is the size of output image, to ensure there the input image size is greater than 4x4
        # Input image size (random noise data) is determined by the times of upsampling
        self.init = init
    ...
    def _evaluation(self, pop, eval_flag):
        ...
        for i in active_index:
            ...
            upsample_num_list.append(pop[i].check_upsample())

        # print('upsample_num_list')
        # print(upsample_num_list)
        # evaluation
        fp = self.eval_func(net_lists, upsample_num_list)
    ...
    def modified_evolution(self, max_eval=100, mutation_rate=0.01, log_file='./log.txt', arch_file='./arch.txt'):
        with open('child.txt', 'w') as fw_c :
            ...
            upsample_num= self.pop[0].check_upsample()
            if self.init:
                pass
            else: # in the case of not using an init indiviudal
                # 如果生成的genotype不合法，重新生成
                while active_num < self.pop[0].net_info.min_active_num or upsample_num > self.max_upsample_num or upsample_num < 1:
            ...
            while self.num_gen < max_eval:
                ...
                for i in range(self.lam):
                    ...
                    upsample_num= self.pop[i + 1].check_upsample()
                    # mutation (forced mutation)
                    while not eval_flag[i] or active_num < self.pop[i + 1].net_info.min_active_num or upsample_num > self.max_upsample_num or upsample_num < 1:
                        self.pop[i + 1].copy(self.pop[0])                       # copy a parent
